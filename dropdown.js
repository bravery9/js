...
function dropdown(_application, _textField, _fieldName, _popupDropField,selectionDisplay)
{
	...
	if (_popupDropField != "" && selectionDisplay != "Yes")
	{
...
function enquiryDropdown(thisEvent)
{
	...
	if ( ( popupDropField == undefined ) || ( popupDropField == null ) )
    {
		popupDropField = "";
    }
	...
	if (popupDropField != "" && selectionDisplay !="Yes")
 	{
 		// save global variable to be restored later
 		savepopupDropDown_GLOBAL = popupDropDown_GLOBAL;
 		//To restore hot field attribute for IE
...
function enquiryBuildDropdown(thisEvent)
{
	...
	// When enquiry selection window is opened using enquiry with build routine attached. 
	// WS_parentWindow is set to composite screen, windowName is set to new result  window
	// We also require to pass the current enquiry window name for donochangefield checks. This will ensure no impacts on the current working. 		
	var enqBuildParentElement = document.createElement("input");
	enqBuildParentElement.type = "hidden";
	enqBuildParentElement.name = "enqBuildParentWindow";
	enqBuildParentElement.value = FragmentUtil.getWindowOrFragmentName(); 		 		 		 	 		 		
	appreqForm.appendChild(enqBuildParentElement);			
...
function pick(_v, _fieldName, _enrichment)
{
  ...
  // Get current window.name to update parent window
	 ...
	 if ( dealField != null )
	{
		...
		if (dealField.getAttribute("tabname") == parentWin.currentTabName_GLOBAL)
		{
	    		dealField.focus();
		}
			...
			for ( var i = 0;(i < form.elements.length) && (!found); i++ )
			{
				...
				if ((field != null) && (field.name == _fieldName))
				{
					...
					if ( (field.disabled == false  &&  field.type != "hidden") || (dropEnriObject.className != null && dropEnriObject.className != '' && dropEnriObject.className.indexOf("enrichmentonly") != -1) )
					{
						...
						field.value = _value;
						var activeTab = parentWin.currentTabName_GLOBAL;
						...
						{
							if (field.getAttribute("tabname") == activeTab)
							{
								field.focus();
							}
						} else
...
function hidePopupDropDown(dropFieldId) {
	...
	var dropFieldObjLen = document.getElementsByName(dropFieldId).length;

	if ((dropFieldObjFoc != null && (dropFieldObjFoc.getAttribute("tabname") != currentTabName_GLOBAL)) ||
	   ((dropFieldObjLen > 1) && (dropFieldObjFoc.getAttribute("tabname") == currentTabName_GLOBAL)))
	{	
	...
	if (dropFieldObjFoc != null )
	{
		try
		{
			...
			{
				setTimeout(function() { dropFieldObjFoc.focus(); }, 100); 
			}
