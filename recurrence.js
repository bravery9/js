...
{
	var recFieldObj = getCurrentTabField( currentForm_GLOBAL, fieldName );
	// If the field is not in the form just try a getElementById()
	if (!recFieldObj) 
	{
		recFieldObj = document.getElementById(fieldName);
	}
	if (recFieldObj != null )
	{
		try 
		{
			...
			{
				setTimeout(function() { recFieldObj.focus(); }, 100);
			}
		} 
		catch( e)
		{ 
...
function rec_saveDateonKey(fieldName,thisEvent)
{
	...
	var recSaveObj = getCurrentTabField( currentForm_GLOBAL, fieldName );
	// If the field is not in the form just try a getElementById()
	if (!recSaveObj) 
	{
		recSaveObj = document.getElementById(fieldName);
	}
	if (recSaveObj != null )
	{
		try 
		{
			...
			if ((navigator.userAgent.toLowerCase().indexOf("chrome") != -1)|| (navigator.userAgent.toLowerCase().indexOf("firefox") != -1))
			{
				setTimeout(function() { recSaveObj.focus(); }, 100);
			}
		}
		catch( e)
		{
