...
/**
 * Replace all occurrences of special characters with the html code
 * @param (String) str
 * @return 'str'
 */
...
function setPassword()
{
	...
	if ( ( action == _PROCESS__REPEAT_ ) || ( action == _PROCESS__EXPIRED_ ) )
	{
        ...
        encUserName = cpForm.encSignOnName.value;
 		...
 		// Changing special characters to html entity in password.
 		oldPassword = escapeSpecialChars(oldPassword);
	...
	else if ( action == _PROCESS__CHANGE_ )
	{
		...
		// Changing special characters to html entity in password.
		password = escapeSpecialChars(password);
		newPassword = escapeSpecialChars(newPassword);
		confirmNewPassword = escapeSpecialChars(confirmNewPassword);
...
function focusField(fieldName)
{
	...
	if(focusFld){
		...
		//check whether the current tab id exists so that focus can be set
		var currentTabId = getFormField(currentForm_GLOBAL, "reqTabid");		
		if ((currentTabId != null) && (currentTabId != "undefined") && (currentTabId != "" )) 
		{
			var fieldList = getFormFields(currentForm_GLOBAL, fieldName);
			for (var i = 0; i < fieldList.length; i++) 
			{
				if ((fieldList[i].getAttribute("tabname")) == currentTabId.value) 
				{
					var focusFld = fieldList[i];
				}
			}
		}
	...
	if ( fieldTabName != null )
	{
		if (fieldTabName != "")
		{
			...
			if ( ( fieldTable != null ) && ( fieldTable.style.visibility != "hidden" ) && ( fieldTableDisabled == "false" ))
			{
				...
				if(trDetails != '')
				{
					// Irrespective of Internet Browser find the action_container in the first field to set focus on the first field
					for( var childFieldCount  = 0; childFieldCount < trDetails.childNodes.length; childFieldCount++ )
					{
						var childNodeOfField = trDetails.childNodes[childFieldCount];

						if (childNodeOfField.className != null)
						{
							var isActionContainer = childNodeOfField.className.split(' ');
							if( isActionContainer[0] == 'action_container') {
								actionContainerDets = trDetails.childNodes[childFieldCount];
								break;
							}
						}
					}					
...
function invokeHelp(evt,fieldName)
{
	...
	if (arguments.length > 2) {
		tokenParam = arguments[2];
	}
	var charCode = (evt.which) ? evt.which : event.keyCode;
	if (evt.altKey && charCode == 49) {
		help(fieldName,"","",tokenParam);
	}
}
