...
FieldGroup.updateHref = function( aElement, incrementBy)
{
	...
	if( FieldGroup.isMultiValue )
	{
		...
		if( hrefStr.indexOf("javascript:doFieldContext(") > -1 )
		{
			var fname = FieldGroup.splitAt( hrefStr, "'", 7);
			if( (fname != null) && (fname.indexOf("fieldName:") == 0) )
			{
				var tokenArray = fname.split(":");
				var beforeIncrement = ":" + tokenArray[2];
				
				tokenArray[2]= (tokenArray[2]*1) + incrementBy;
				var afterIncrement = ":" + tokenArray[2];
				var regex = new RegExp(beforeIncrement , "g"); 
				
				hrefStr = hrefStr.replace(regex, afterIncrement);
				aElement.setAttribute("href", hrefStr);
			}	
		}
	}
