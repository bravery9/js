			...
			for(var p = tdElement.parentNode; p && p.nodeName != "body"; p = p.parentNode)
			{
				if(p.className != null && p.className.indexOf("fragmentContainer") != -1)
				{
					if(p.className.indexOf("printableFragment") == -1)
					{
						...
						//fix to eliminate content overlapping in firefox during print
						if(isFirefox())
						{
...
function drilldown( item, ref )
{
	...
	if(FragmentUtil.isCompositeScreen())
	{
		...
		//When T24 respond with SMS error page in composite screen, clear the drilltarget value explicitly to avoid screen freezing issue.
		var errorMessageFromT24;
		var drilltarget;
		if(top.noFrames)
		{
			errorMessageFromT24 = document.getElementById("errorMessageFromT24");
		}
		else
		{
			for ( var i = 0; i < window.parent.frames.length; i++ )                          
			{                                                                                
				errorMessageFromT24 = window.parent.frames[i].document.getElementById("errorMessageFromT24");
			}
		}
		if( errorMessageFromT24 !=null )
		{
			if (errorMessageFromT24.value !=null && errorMessageFromT24.value !="")
			{
				if(top.noFrames){
					drilltarget = document.getElementsByName("drilltarget");
					for(var i=0;i<drilltarget.length;i++){
						if((drilltarget[i].value !=null) && (drilltarget[i].value !=''))
							drilltarget[i].value = '';
					}
					errorMessageFromT24.value = '';
				}
				else
				{
					for ( var i = 0; i < window.parent.frames.length; i++ )                          
					{                                                                                
						drilltarget = window.parent.frames[i].document.getElementById("drilltarget");
						if((drilltarget.value !=null) && (drilltarget.value !=''))
							drilltarget.value = '';
					}
					errorMessageFromT24.value = '';
				}
...
function resizeEnquiry() {
	...
	if ( (tabHeader) && (getChildElementFromParent("pane_"+ currentForm_GLOBAL, "enquiryHeadingScroller", "div")) )
	{
		headingDiv = getChildElementFromParent("pane_"+ currentForm_GLOBAL, "enquiryHeadingScroller", "div");
		headingSizer = getChildElementFromParent("pane_"+ currentForm_GLOBAL, "enquiryHeadingSizer", "div");
		dataSizer = getChildElementFromParent("pane_"+ currentForm_GLOBAL, "enquiryDataSizer", "div");
		dataDiv = getChildElementFromParent("pane_"+ currentForm_GLOBAL, "enquiryDataScroller", "div");		
	}
	else
	{		
		headingDiv = FragmentUtil.getElement("enquiryHeadingScroller");
		headingSizer = FragmentUtil.getElement("enquiryHeadingSizer");
		dataSizer = FragmentUtil.getElement("enquiryDataSizer");
		dataDiv = FragmentUtil.getElement("enquiryDataScroller");		
	}
