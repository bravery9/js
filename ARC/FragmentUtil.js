...
//			     	}
    ...
    isWindow : function(obj) {
    	...
    	// Microsoft EDGE raises 'remote server machine does not exist or is unavailable' error for resizeBy if window is closed.
    	// Chrome and other browsers ignore this error. So introducing condition for edge browser. 
    	if (isEdge())     	  	
    ...
    keydownHandler : function(theEvent, fragmentName) {
    	...
    	if (theEvent.type == "keydown") 
    	{
    		...
    		// Ctrl + P needs to print the whole page. So set the display type for all non printable fragment to table
    		// block display type causes content overlay in AA.OVERVIEW screen. Hence introducing the table display type.
	    	if (FragmentUtil.isCompositeScreen() && top.noFrames == true) 
	    	{
	    		if (theEvent.ctrlKey && theEvent.keyCode == 80) 
	    		{	    		
	    			var frags = document.querySelectorAll('.fragmentContainer.notPrintableFragment'), i;
	    			for (i = 0; i < frags.length; ++i) 
	    			{ 
	    				frags[i].style.display = "table";
	    			} 
	    		}
	    	}
    	}
