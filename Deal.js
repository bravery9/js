 ...
 */ 
 function toggleCheckBox(thisEvent)
 {
	 ...
	 if ( checkbox.checked == true )
	 {
		 ...
		 // if more checkbox available in screen with same name               
		 if (hiddenCheckbox.length > 0) 
		 {                             
			 for(var i=0; i < hiddenCheckbox.length; i++) 
			 {
				 hiddenCheckbox[i].value = checkbox.getAttribute("checkedValue");
				 var check = getFormFields( tabList_GLOBAL[i], "CheckBox:" + checkboxName);
				 // make all other check box as checked
				 if (!check[0].checked) 
				 {
	 ...
	 else
	 {
		 ...
		 {                             
			 for(var i=0; i < hiddenCheckbox.length; i++) 
			 {
				 hiddenCheckbox[i].value = checkbox.getAttribute("uncheckedValue");
				 var check = getFormFields( tabList_GLOBAL[i], "CheckBox:" + checkboxName);
				 // make all other check box as unchecked
				 if (check[0].checked) 
				 {
...
function setEnrichment( _fieldName, _enrichment)
{
	...
	var checkbox = "CheckBox:" + _fieldName; 
	var checkfieldLength = document.getElementsByName(checkbox).length;
	var checkname =document.getElementsByName(checkbox);
	var enriObject = "";
	//to display checkbox enrichment if it is present in one or more tabs 
	if(checkfieldLength>0)
	{
		for(var i=0; i< checkfieldLength;i++)
		{

			if((checkname[i].getAttribute("tabname")) !=null)
			{
				enriObject = getTabEnrichmentSpan(checkname[i].getAttribute("tabname"), enriFieldName );
			}
			if(enriObject != "")
			{
				displayEnrichment(enriObject,_enrichment);
			}
		}
	}
	else
	{
		enriObject = getTabEnrichmentSpan(currentForm_GLOBAL, enriFieldName );
		if(enriObject != "")
		{
			displayEnrichment(enriObject,_enrichment);
		}
...
function doFieldChangeEvent(thisEvent)
{
	...
	{
		var currentTabId = thisField.getAttribute("tabname");
		setFormFieldValue(currentForm_GLOBAL, "reqTabid", currentTabId);
		if ( ( isHotVal == "Y" ) && ( isHot == "Y" ) )
		{
...
/**
 * Check if we have a enrichment field with the correct name and display the enrichment
 * 
 * @return void
 * @type null
 */
function displayEnrichment (enriObject, _enrichment)
{
	if ( enriObject != null )
	{	
		if (isOpera())
